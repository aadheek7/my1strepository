...
print("\n\nPreparation of Sotres Ledger Account without Returns and Losses Under FIFO method...")
while True:
...
-------------------------------------------->>> """)
    ...
    if choice.lower() == "g":
        ...
        v_scroll = ttk.Scrollbar(mainFrame,orient = VERTICAL,command = canvas.yview); v_scroll.pack(side = RIGHT,fill = Y)
        h_scroll = ttk.Scrollbar(root,orient = HORIZONTAL,command = canvas.xview); h_scroll.pack(side = BOTTOM, fill = X)
        canvas.configure(
			yscrollcommand = v_scroll.set,
			xscrollcommand = h_scroll.set)
        projectFrame = Frame(canvas)
        projectFrame.bind("<Configure>",lambda e: canvas.configure(scrollregion = canvas.bbox("all")))
        canvas.create_window((0, 0),window = projectFrame,anchor = "nw")
        Label(projectFrame,text = stores_ledger_account_table,font = ("consolas", 16), relief = "solid").pack()
        root.mainloop()
        balance_amount = 0
        tot_units = 0
        for i in balance_list:
        	balance_amount += i[2]
        	tot_units += i[0]
        print("Closing Stock =", tot_units, "units, valued at Rs.", balance_amount, "\n\n")
